...
class FileAudioQueue:
  ...
  def __init__(self, capacity_seconds: float = FILE_QUEUE_CAPACITY_SECONDS) -> None:
    ...
    self._capacity_samples: int = int(capacity_seconds * CANONICAL_SAMPLE_RATE_HZ)
    self._chunks: deque[Float32Audio] = deque()
    self._queued_samples: int = 0
    self._producer_done: bool = False
    self._condition: asyncio.Condition = asyncio.Condition()
    self._total_enqueue_block_s: float = 0.0

  async def enqueue(self, chunk: Float32Audio) -> float:
  ...
  async def dequeue(self) -> Float32Audio | None:
