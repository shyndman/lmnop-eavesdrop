...
class BatchedInferencePipeline:
      ...
      segmented_outputs.append(
        [
          dict(
            ...
            compression_ratio=get_compression_ratio(tokenizer.decode(subsegment["tokens"])),
            seek=int(chunk_metadata["start_time"] * self.model.frames_per_second),
          )
  ...
  def generate_segment_batched(
    ...
    prompt = self.model.get_prompt(
      tokenizer,
      previous_tokens=(
        tokenizer.encode(options.initial_prompt) if options.initial_prompt is not None else []
      ),
  ...
  def transcribe(
    ...
    if not clip_timestamps:
      if vad_filter:
        ...
        self.model.logger.info(
          "Configured BatchedInferencePipeline VAD options: max_speech_duration_s=%.1f, "
          "min_silence_duration_ms=%d, speech_pad_ms=%d, threshold=%.3f, "
          "min_speech_duration_ms=%d, window_size_samples=%d",
          ...
          vad_parameters.window_size_samples,
        )
    ...
    duration_after_vad = (
      sum((segment["end"] - segment["start"]) for segment in clip_timestamps) / sampling_rate
    )
      ...
      if not self.model.model.is_multilingual:
        ...
        (
          ...
          features=np.concatenate(
            features + [np.full((self.model.model.n_mels, 1), -1.5, dtype="float32")],
            axis=1,
    ...
    options = TranscriptionOptions(
      ...
      temperatures=(temperature[:1] if isinstance(temperature, (list, tuple)) else [temperature]),
      ...
      for result in results:
        for segment in result:
          ...
          yield Segment(
            ...
            words=(
              None if not options.word_timestamps else [Word(**word) for word in segment["words"]]
            ),
...
class WhisperModel:
      ...
      for segment in current_segments:
        ...
        all_segments.append(
          Segment(
            ...
            no_speech_prob=result.no_speech_prob,
            words=(
              [Word(**word) for word in segment["words"]] if options.word_timestamps else None
            ),
          )
        )
  ...
  def add_word_timestamps(
    ...
    for segment in segments:
      segment_tokens = [
        [token for token in subsegment["tokens"] if token < tokenizer.eot] for subsegment in segment
      ]
