...
class ServeClientBase(object):
  ...
  def __init__(
    ...
    self.logger = get_logger("serve_client_base")
  ...
  def clip_audio_if_no_valid_segment(self):
    ...
    """
    with self.lock:
      assert self.frames_np is not None
      if (
        self.frames_np[int((self.timestamp_offset - self.frames_offset) * self.RATE) :].shape[0]
        > 25 * self.RATE
      ):
  ...
  def update_segments(self, segments, duration):
    ...
    if self.same_output_count > self.same_output_threshold:
      if not self.text or self.text[-1].strip().lower() != self.current_out.strip().lower():
        ...
        with self.lock:
          completed_segment = self.format_segment(
            ...
            self.timestamp_offset + min(duration, self.end_time_for_same_output),  # type: ignore
            self.current_out,
            completed=True,
          )
          self.transcript.append(completed_segment)
      ...
      self.current_out = ""
      offset = min(duration, self.end_time_for_same_output)  # pyright: ignore[reportArgumentType]
      self.same_output_count = 0
      last_segment = None
      self.end_time_for_same_output = None
    else:
