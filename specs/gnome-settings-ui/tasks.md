...
- [ ] 1.1a Validate the GNOME package builds and typechecks with the new prefs entrypoint (`pnpm build`, `pnpm typecheck`), and confirm the preferences window opens with `gnome-extensions prefs eavesdrop@shyndman.ca`. (HUMAN_REQUIRED)
...
- [ ] 2.1a Verify the prefs window loads fallback prompt contents when the override file is missing, writes raw contents to `~/.config/active-listener/system.md` on save, and restores on revert. (HUMAN_REQUIRED)
...
- [ ] 4.1 Verify the manual workflow end to end: edit the system prompt in GNOME prefs, save it, trigger two rewrite requests without restarting `active-listener`, and confirm the later request uses the updated prompt. (HUMAN_REQUIRED)
